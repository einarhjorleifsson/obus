...
| `"IDX"` | `icesDatras::getIndices` | Age-based survey indices; `aphia` required (or defaults); iterates per survey×year×quarter×species (all scalar) |
...
| `.dr_fetch_cpue_age(surveys, years, quarters, quiet)` | Same pattern; strips `xsi:nil="true"` and coerces Age_* **per data frame before `bind_rows`** to avoid duplicates across species |
...
| `.dr_fetch_indices(surveys, years, quarters, aphia, quiet)` | `expand_grid` × `pmap` per survey×year×quarter×species (all scalar); same Age_* cleanup as CPUEA |
...
- **CPUEA / IDX XML nil artifact:** Age columns with no data arrive with names like `` `Age_6 xsi:nil="true"` ``. The fetcher strips this suffix and coerces Age_* to numeric **per data frame before `bind_rows`** — if done after, different age structures across species produce duplicate column names.
...
- **IDX returns no rows silently:** If a species has no indices defined for a given survey/quarter (e.g. herring in NS-IBTS Q1), `tryCatch` drops the result silently. This is expected behaviour.
